...
function undoWithOps(state) {
  ...
  return [combine(applyOp(state, operations), state.history, transaction - 1, nextVersion(state)), operations];
}
...
function redoWithOps(state) {
  ...
  return [combine(applyOp(state, operations), state.history, transaction, nextVersion(state)), operations];
}
